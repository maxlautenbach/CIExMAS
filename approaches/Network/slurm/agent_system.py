...
from approaches.Network.setup import cIEState
from approaches.Network.agents.extractor import agent as extractor
from approaches.Network.agents.uri_mapping_and_refinement import agent as uri_mapping_and_refinement
from approaches.Network.agents.validation_and_output import agent as validation_and_output
from approaches.Network.agents.uri_search_tool import agent as uri_search_tool
from approaches.Network.agents.network_traversal_search import agent as network_traversal_search
from approaches.Network.agents.turtle_to_labels_tool import agent as turtle_to_labels_tool
from approaches.Network.agents.semantic_triple_checking_tool import agent as semantic_triple_checking_tool
...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({
            ...
            "call_trace": [],
            "uri_mapping": "",
        }, config={"run_id": trace_id, "recursion_limit": 40, "callbacks": [langfuse_handler], "tags":["Network", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/approaches/evaluation_logs/Network/{dataset}-{split}-{number_of_samples}-evaluation_log-{os.getenv('LLM_MODEL_PROVIDER')}_{os.getenv('LLM_MODEL_ID').replace('/', '-')}-{datetime.now().strftime('%Y-%m-%d-%H%M')}.xlsx"
try:
