...
- **Predicate Extraction Agent:** Extracts predicates from the text. Can handle addition instructions to make a disambiguation. Is an expert for predicates in knowledge graphs.
- **Triple Extraction Agent:** Extracts triples from the text. Can handle addition instructions to make a disambiguation. Is an expert for triples in knowledge graphs.
...
In addition, you will receive the history of agent call traces and the text which is being processed. Your task is then to execute the next step and instruct the next agent as adviced by the planner in the comments. Please provide reasoning.
    ...
    Predicate Extraction Agent
    - id: predicate_extraction_agent
    - use of instruction: The use of an instruction is optional. It will be included in the context of the prompt of the agent and can modify the agents behaviour. Please do not include the original text in the prompt. It can be relevant to provide the predicate extraction agent with already extracted entities or entities it should focus on.
    - description: Can extract predicates from the text.
    - has access on: text, instruction

    Triple Extraction Agent
    - id: triple_extraction_agent
    - use of instruction: The use of an instruction is optional. It will be included in the context of the prompt of the agent and can modify the agents behaviour. Please do not include the original text in the prompt. It can be relevant to provide the triple extraction agent with already extracted entities or entities it should focus on.
    - description: Can extract triples from the text.
    - has access on: text, instruction
...
triple_extractor_prompt = PromptTemplate.from_template("""
    ...
    You are an expert for triple extraction out of text in a multi-agent-system for closed information extraction. You will receive a text out of the state from which you should extract all triples. As closed information extraction uses an underlying knowledge graph, there can be different names for similar predicates. Therefore, extract also alternative predicates, when applicable (i.e. Berlin, located in, Germany -> Berlin, country, Germany). 
