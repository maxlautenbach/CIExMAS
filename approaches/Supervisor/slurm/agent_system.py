...
from approaches.Supervisor.setup import cIEState
from approaches.Supervisor.agents.planner import agent as planner
from approaches.Supervisor.agents.entity_extractor import agent as entity_extraction_agent
from approaches.Supervisor.agents.predicate_extractor import agent as predicate_extraction_agent
from approaches.Supervisor.agents.triple_extractor import agent as triple_extraction_agent
from approaches.Supervisor.agents.uri_retriever import agent as uri_retriever_agent
from approaches.Supervisor.agents.turtle_extractor import agent as turtle_extraction_agent
...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({
            ...
            "instruction": "",
            "debug": False
        }, config={"run_id": trace_id, "recursion_limit": 70, "callbacks": [langfuse_handler], "tags":["Supervisor", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/approaches/evaluation_logs/Supervisor/{dataset}-{split}-{number_of_samples}-evaluation_log-{os.getenv('LLM_MODEL_PROVIDER')}_{os.getenv('LLM_MODEL_ID').replace('/', '-')}-{datetime.now().strftime('%Y-%m-%d-%H%M')}.xlsx"
try:
