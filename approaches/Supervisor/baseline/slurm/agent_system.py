...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({"text": text, "messages": [], "debug": False},
                                config={"run_id": trace_id, "recursion_limit": 35, "callbacks": [langfuse_handler], "tags":["Baseline", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/approaches/evaluation_logs/baseline/{dataset}-{split}-{number_of_samples}-evaluation_log-{os.getenv('LLM_MODEL_PROVIDER')}_{os.getenv('LLM_MODEL_ID').replace('/','-')}-{datetime.now().strftime('%Y-%m-%d-%H%M')}.xlsx"
try:
