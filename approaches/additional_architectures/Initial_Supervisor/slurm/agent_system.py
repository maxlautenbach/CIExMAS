...
from approaches.additional_architectures.Initial_Supervisor.setup import cIEState
from approaches.additional_architectures.Initial_Supervisor.agents.supervisor import agent as supervisor_agent
from approaches.additional_architectures.Initial_Supervisor.agents.entity_extractor import agent as entity_extraction_agent
from approaches.additional_architectures.Initial_Supervisor.agents.relation_extractor import agent as relation_extraction_agent
from approaches.additional_architectures.Initial_Supervisor.agents.uri_detector import agent as uri_detection_agent
...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({"text": text, "messages": [], "debug": False},
                                config={"run_id": trace_id, "recursion_limit": 35, "callbacks": [langfuse_handler], "tags":["Initial_Supervisor", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/approaches/evaluation_logs/additional/Initial_Supervisor/{dataset}-{split}-{number_of_samples}-evaluation_log-{os.getenv('LLM_MODEL_PROVIDER')}_{os.getenv('LLM_MODEL_ID').replace('/','-')}-{datetime.now().strftime('%Y-%m-%d-%H%M')}.xlsx"
try:
