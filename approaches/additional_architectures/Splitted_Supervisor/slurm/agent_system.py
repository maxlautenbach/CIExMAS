...
from approaches.additional_architectures.Splitted_Supervisor.setup import cIEState
from approaches.additional_architectures.Splitted_Supervisor.agents.agent_instructor import agent as agent_instructor_agent
from approaches.additional_architectures.Splitted_Supervisor.agents.entity_extractor import agent as entity_extraction_agent
from approaches.additional_architectures.Splitted_Supervisor.agents.relation_extractor import agent as relation_extraction_agent
from approaches.additional_architectures.Splitted_Supervisor.agents.uri_detector import agent as uri_detection_agent
from approaches.additional_architectures.Splitted_Supervisor.agents.result_checker import agent as result_checker_agent
from approaches.additional_architectures.Splitted_Supervisor.agents.result_formatter import agent as result_formatting_agent
from approaches.additional_architectures.Splitted_Supervisor.agents.planner import agent as planner
...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({"text": text, "results": [], "call_trace": [], "comments": [], "debug": False},
                            config={"run_id": trace_id, "recursion_limit": 70, "callbacks": [langfuse_handler], "tags":["Splitted_Supervisor", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/approaches/evaluation_logs/additional/Splitted_Supervisor/{dataset}-{split}-{number_of_samples}-evaluation_log-{os.getenv('LLM_MODEL_PROVIDER')}_{os.getenv('LLM_MODEL_ID').replace('/', '-')}-{datetime.now().strftime('%Y-%m-%d-%H%M')}.xlsx"
try:
