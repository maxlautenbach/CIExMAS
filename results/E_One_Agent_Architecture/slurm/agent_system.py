...
from results.E_One_Agent_Architecture.setup import cIEState
from results.E_One_Agent_Architecture.agents.main_agent import agent as main
from results.E_One_Agent_Architecture.agents.uri_search_tool import agent as uri_search_tool
...
for i in tqdm(range(len(docs))):
    ...
    try:
        ...
        response = graph.invoke({"text": text, "messages": [], "instruction": "", "debug": False},
                                config={"run_id": trace_id, "recursion_limit": 70, "callbacks": [langfuse_handler], "tags":["E_One_Agent_Architecture", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/results/result_evaluation_logs/E_One_Agent_Architecture.xlsx"
try:
