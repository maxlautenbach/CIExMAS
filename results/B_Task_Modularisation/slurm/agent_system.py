...
from results.B_Task_Modularisation.setup import cIEState
from results.B_Task_Modularisation.agents.agent_instructor import agent as agent_instructor_agent
from results.B_Task_Modularisation.agents.entity_extractor import agent as entity_extraction_agent
from results.B_Task_Modularisation.agents.relation_extractor import agent as relation_extraction_agent
from results.B_Task_Modularisation.agents.uri_detector import agent as uri_detection_agent
from results.B_Task_Modularisation.agents.result_checker import agent as result_checker_agent
from results.B_Task_Modularisation.agents.result_formatter import agent as result_formatting_agent
from results.B_Task_Modularisation.agents.planner import agent as planner
...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({"text": text, "results": [], "call_trace": [], "comments": [], "debug": False},
                            config={"run_id": trace_id, "recursion_limit": 70, "callbacks": [langfuse_handler], "tags":["B_Task_Modularisation", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/results/result_evaluation_logs/B_Task_Modularisation.xlsx"
try:
