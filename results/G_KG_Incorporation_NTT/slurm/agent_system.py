...
from results.G_KG_Incorporation_NTT.setup import cIEState
from results.G_KG_Incorporation_NTT.agents.main_agent import agent as main
from results.G_KG_Incorporation_NTT.agents.uri_search_tool import agent as uri_search_tool
from results.G_KG_Incorporation_NTT.agents.network_traversal_search import agent as network_traversal_search
...
for i in tqdm(range(len(docs))):
    ...
    try:
        ...
        response = graph.invoke({"text": text, "messages": [], "instruction": "", "debug": False},
                                config={"run_id": trace_id, "recursion_limit": 70, "callbacks": [langfuse_handler], "tags":["G_KG_Incorporation_NTT", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/results/result_evaluation_logs/G_KG_Incorporation_NTT.xlsx"
try:
