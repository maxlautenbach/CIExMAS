...
from results.D_System_Simplification.setup import cIEState
from results.D_System_Simplification.agents.planner import agent as planner
from results.D_System_Simplification.agents.entity_extractor import agent as entity_extraction_agent
from results.D_System_Simplification.agents.predicate_extractor import agent as predicate_extraction_agent
from results.D_System_Simplification.agents.triple_extractor import agent as triple_extraction_agent
from results.D_System_Simplification.agents.uri_retriever import agent as uri_retriever_agent
from results.D_System_Simplification.agents.turtle_extractor import agent as turtle_extraction_agent
...
for i in tqdm(range(len(docs))):
    ...
    try:
        response = graph.invoke({
            ...
            "instruction": "",
            "debug": False
        }, config={"run_id": trace_id, "recursion_limit": 70, "callbacks": [langfuse_handler], "tags":["D_System_Simplification", f'{os.getenv("LLM_MODEL_PROVIDER")}-{os.getenv("LLM_MODEL_ID")}']})
...
excel_file_path = f"{repo.working_dir}/results/result_evaluation_logs/D_System_Simplification.xlsx"
try:
