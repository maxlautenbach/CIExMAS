...
All URIs in turtle must be enclosed in angle brackets. Ignore your implicit knowledge about public knowledge graphs (i.e. Namespaces for properties or URIs mapped to labels) and make sure, that you only use URIs, that were previously extracted by the uri_detection_agent. If you can't find a corresponding URI for a predicate or entity, please don't output this triple, as such triples will be considered as invalid. Only use the @prefix wd: <http://www.wikidata.org/entity/> . for the wikidata namespace, also for properties.
